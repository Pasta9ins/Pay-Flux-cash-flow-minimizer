...
void printAns(vector<vector<pair<int, string>>> ansGraph, int numBanks, bank input[]) {
    ...
    ofstream graphFile("cash_flow_graph.dot");
    graphFile << "digraph CashFlow {\n";
    ...
    for (int i = 0; i < numBanks; i++) {
        for (int j = 0; j < numBanks; j++) {
            ...
            if (ansGraph[i][j].first != 0) {
                ...
                graphFile << "\t" << input[i].name << " -> " << input[j].name 
                          << " [label=\"Rs " << ansGraph[i][j].first << " (" << ansGraph[i][j].second << ")\"];\n";
            }
        }
    }
    ...
    graphFile << "}\n";
    graphFile.close();
    cout << "\nGraph data has been exported to 'cash_flow_graph.dot'. Use Graphviz to visualize it.\n";
}
...
void minimizeCashFlow(int numBanks, bank input[], unordered_map<string, int>& indexOf, int numTransactions, vector<vector<int>>& graph, int maxNumTypes) {
    ...
    while (numZeroNetAmounts != numBanks) {
        ...
        if (maxIndex == -1) {
            ...
            (ansGraph[minIndex][simpleMaxIndex].first) += amount;
            (ansGraph[minIndex][simpleMaxIndex].second) = *(input[minIndex].types.begin());
